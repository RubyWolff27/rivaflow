export const CLASS_TYPES = [
  { value: 'gi', label: 'Gi', color: '#3B82F6', primary: true },
  { value: 'no-gi', label: 'No-Gi', color: '#8B5CF6', primary: true },
  { value: 'open-mat', label: 'Open Mat', color: '#F59E0B', primary: false },
  { value: 'competition', label: 'Competition', color: '#EF4444', primary: false },
  { value: 's&c', label: 'S&C', color: '#10B981', primary: false },
  { value: 'cardio', label: 'Cardio', color: '#06B6D4', primary: false },
  { value: 'mobility', label: 'Mobility', color: '#EC4899', primary: false },
  { value: 'drilling', label: 'Drilling', color: '#F97316', primary: false },
];

const PRIMARY_TYPES = CLASS_TYPES.filter(t => t.primary);
const SECONDARY_TYPES = CLASS_TYPES.filter(t => !t.primary);
...
export default function ClassTypeChips({
  ...
  // Multi-select mode: primary (single-select) + secondary (multi-select)
  return (
    <div className="space-y-2">
      {/* Primary: Gi or No-Gi (required, single-select) */}
      <div className="flex flex-wrap gap-2" role="group" aria-label="Primary class type">
      ...
      {/* Secondary: additional tags (optional, multi-select) */}
