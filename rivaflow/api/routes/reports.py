...
async def get_week_report(target_date: date = None, current_user: dict = Depends(get_current_user)):
    ...
    report = service.generate_report(user_id=current_user["id"], start_date=start_date, end_date=end_date)
    return report


@router.get("/month")
async def get_month_report(target_date: date = None, current_user: dict = Depends(get_current_user)):
...
async def get_range_report(start_date: date, end_date: date, current_user: dict = Depends(get_current_user)):
