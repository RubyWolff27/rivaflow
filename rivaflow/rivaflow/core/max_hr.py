...
def calibrate_max_hr(
    hr_samples: list[int],
    age: float,
    plateau_sec: int = DEFAULT_PLATEAU_SEC,
    hz: float = 1.0,
) -> dict:
    ...
    hr = [
        int(x) for x in hr_samples if x is not None and HR_FLOOR <= int(x) <= HR_CEILING
    ]
    ...
    if observed is not None and observed >= tanaka:
        ...
        note = (
            "Observed sustained max at/above age-predicted — using the measured value."
        )
    else:
        estimate = tanaka
        ...
        if floor:
            note = (
                f"Observed sustained max {observed} is below age-predicted {round(tanaka)} — likely "
