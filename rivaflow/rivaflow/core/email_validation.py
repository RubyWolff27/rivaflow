...
def validate_email_address(
    email: str, check_deliverability: bool = False, allow_disposable: bool = False
) -> tuple[bool, str, dict]:
    """
    ...
    if not email:
        return (
            False,
            "",
            {"code": "EMPTY_EMAIL", "message": "Email address is required"},
        )
    ...
    try:
        ...
        return (
            False,
            email,
            {"code": error_code, "message": message, "details": error_message},
        )
...
def is_valid_email_simple(email: str, allow_disposable: bool = False) -> bool:
    ...
    is_valid, _, _ = validate_email_address(
        email, check_deliverability=False, allow_disposable=allow_disposable
    )
    return is_valid
