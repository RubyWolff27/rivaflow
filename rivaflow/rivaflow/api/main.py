            ...
            with get_connection() as conn:
                ...
                for table, col, col_def in _ensure_columns:
                    ...
                    if cur.fetchone() is None:
                        ...
                        cur.execute(f"ALTER TABLE {table}" f" ADD COLUMN {col} {col_def}")
                        conn.commit()
                        logging.info("Added %s.%s column", table, col)
                # Ensure needs_review is BOOLEAN (may be INTEGER
                # on old production if migrate.py didn't run)
                cur.execute(
                ...
                if row and row[0] != "boolean":
                    logging.warning(
                        "needs_review is %s, converting to BOOLEAN",
                        row[0],
                    )
                    cur.execute(
                        "ALTER TABLE sessions "
                        "ALTER COLUMN needs_review TYPE BOOLEAN "
                        "USING CASE WHEN needs_review = 0 "
                        "THEN FALSE WHEN needs_review IS NULL "
