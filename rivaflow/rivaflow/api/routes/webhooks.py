...
def _verify_whoop_signature(body: bytes, signature: str) -> bool:
    """Verify HMAC-SHA256 signature from WHOOP webhook."""
    secret = settings.WHOOP_WEBHOOK_SECRET
    if not secret:
        logger.warning("WHOOP_WEBHOOK_SECRET not set — rejecting webhook")
        return False
...
async def whoop_webhook(request: Request):
    ...
    if not signature or not _verify_whoop_signature(body, signature):
        raise HTTPException(
