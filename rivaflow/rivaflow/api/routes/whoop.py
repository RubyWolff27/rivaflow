...
# /summary's ~35s live cost (see the comment above _SUMMARY_SLOW_MS) is exactly what the
# Wave 2.2 cockpit snapshot exists to avoid: whoop_cockpit_snapshot.metrics_json IS the
# whoop_summary JSON contract, regenerated by the scheduler at 06/09/18/21 Melbourne (and
# on cold-render) and served by /cockpit-metrics in one SELECT. Both /summary call sites
# try that snapshot first and only fall back to a live recompute when it's missing, stale,
# or fails the sabbath guard below.
_SNAPSHOT_MAX_AGE = timedelta(hours=4)  # the scheduler's own cadence ceiling


def _parse_snapshot_timestamp(raw: object) -> datetime | None:
    """Normalise a stored rendered_at to a tz-aware UTC datetime.

    Postgres (prod) returns TIMESTAMPTZ as an already tz-aware datetime via
    psycopg2; SQLite (local dev) returns the ISO string it was inserted as.
    Anything unparseable — including a missing value — returns None.
    """
...
def _summary_from_snapshot(user_id: int) -> dict | None:
    """Serve /summary from the stored cockpit snapshot instead of recomputing
    whoop_summary live. Returns the metrics dict (tagged with source=snapshot
    and as_of=<snapshot timestamp>) when the snapshot is usable, else None —
    the caller falls back to the live path on None.

    A snapshot is usable when it exists, has metrics_json, is no older than
    the scheduler's own 4h cadence ceiling, AND its baked-in sabbath state
    agrees with today's actual rest-day flag. That last check matters: a
    snapshot built Saturday evening (the 18:00 or 21:00 job) served on Sunday
    morning would carry Saturday's non-Rest readiness on the Sabbath, and the
    reverse (a Rest-state snapshot served on a non-rest day) is equally wrong
    — either mismatch means "stale in a way age alone can't catch", so both
    directions fall back to live.
    """
    row = WhoopRepository.get_cockpit_metrics(user_id)
    if not row or not row.get("metrics_json"):
        return None

    rendered_at = _parse_snapshot_timestamp(row.get("rendered_at"))
    if rendered_at is None or datetime.now(UTC) - rendered_at > _SNAPSHOT_MAX_AGE:
        return None
    ...
    try:
        ...
        logger.warning(
            "Snapshot metrics_json unparseable for user %s: %s", user_id, exc
        )
        return None

    is_rest_today = today_is_rest_day(user_id)
    snapshot_says_rest = (metrics.get("readiness") or {}).get("state") == "Rest"
    if is_rest_today != snapshot_says_rest:
        return None

    metrics["source"] = "snapshot"
    metrics["as_of"] = rendered_at.isoformat()
    return metrics


def _live_summary(user_id: int) -> dict:
    """The existing live whoop_summary() path (unchanged, keeps its duration
    logging), tagged with the same additive source/as_of contract as the
    snapshot path."""
    is_sabbath = today_is_rest_day(user_id)
    result = _timed_whoop_summary(user_id, today_is_sabbath=is_sabbath)
    result["source"] = "live"
...
def summary(current_user: dict = Depends(get_current_user)) -> dict:
    ...
    """
    snapshot = _summary_from_snapshot(current_user["id"])
    if snapshot is not None:
        return snapshot
    return _live_summary(current_user["id"])
...
def view(key: str) -> HTMLResponse:
    ...
    snapshot = _summary_from_snapshot(user_id)
    s = snapshot if snapshot is not None else _live_summary(user_id)
