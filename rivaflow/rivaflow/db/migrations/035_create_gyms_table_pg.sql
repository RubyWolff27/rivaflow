...
CREATE TABLE IF NOT EXISTS gyms (
    ...
    website TEXT,
    verified BOOLEAN DEFAULT FALSE,  -- 0 = user-added, 1 = admin-verified
...
-- CREATE INDEX IF NOT EXISTS on name for faster searching
CREATE INDEX IF NOT EXISTS idx_gyms_name ON gyms(name);

-- CREATE INDEX IF NOT EXISTS on city/state for location-based queries
CREATE INDEX IF NOT EXISTS idx_gyms_location ON gyms(city, state);

-- CREATE INDEX IF NOT EXISTS on verified status for filtering
CREATE INDEX IF NOT EXISTS idx_gyms_verified ON gyms(verified);

-- Add primary_gym_id to users table
ALTER TABLE users ADD COLUMN primary_gym_id BIGINT REFERENCES gyms(id) ON DELETE SET NULL;

-- CREATE INDEX IF NOT EXISTS on users.primary_gym_id for faster friend recommendations
